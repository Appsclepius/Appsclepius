...
function widget.newWidgetInfoPanel( options )
    local customOptions = options or {}
    local opt = {}
    ...
    opt.desc = customOptions.desc or ""
...
--    local backgroundPanel = display.newImageRect( "images/ui/background_panel.png", opt.width, opt.height )
--    backgroundPanel.x = display.contentWidth * 0.5
--    backgroundPanel.y = opt.height * 0.5
--    barContainer:insert(backgroundPanel)  

    if opt.image then
        local logoImage = display.newImageRect( opt.image.name, opt.image.width, opt.image.height )
        logoImage.anchorX = 0
        logoImage.anchorY = 0
        logoImage.x = 5
        logoImage.y = 5
        barContainer:insert(logoImage)
    end

    if opt.image then
        local title = display.newText(opt.title, opt.width - opt.image.width, opt.height, opt.titleFont, opt.titleFontSize)
        title:setFillColor(unpack(opt.titleColor))
        title.anchorX = 0
--        title.anchorY = 0
        title.x = opt.image.width+20
        title.y = opt.titleY
        barContainer:insert(title)  
    
--        local desc = display.newText(opt.desc, opt.width - opt.image.width, opt.height, opt.descFont, opt.descFontSize)
--        desc:setFillColor(unpack(opt.descColor))
--        desc.anchorX = 0
----        desc.anchorY = 0
--        desc.x = opt.image.width+20
--        desc.y = opt.descY
--        barContainer:insert(desc)  
    else
        barContainer._title = display.newText(opt.title, opt.titleX, opt.titleY, opt.titleFont, opt.titleFontSize)
        barContainer._title.anchorX = 0
--        barContainer._title.anchorY = 0
        barContainer._title:setFillColor(unpack(opt.titleColor))
        barContainer:insert(barContainer._title)  
     
        barContainer._desc = display.newText(opt.desc, opt.descX, opt.descY, opt.descFont, opt.descFontSize)
...
function widget.newModuleInfoPanel( options )
    local customOptions = options or {}
    local opt = {}
    ...
    local background = display.newRect(barContainer, 2, 2, opt.width - 4, opt.height - 4 + statusBarPad )    
    background:setFillColor( customOptions.backgroundColor[1], customOptions.backgroundColor[2], customOptions.backgroundColor[3] )
    background.anchorX = 0
    background.anchorY = 0
...
--        desc.anchorY = 0
        ...
        barContainer._title.anchorX = 0
--        barContainer._title.anchorY = 0
        barContainer._title.x = 20
        barContainer._title.y = opt.titleY
        barContainer._title:setFillColor(unpack(opt.titleColor))
        barContainer:insert(barContainer._title)  
        ...
        barContainer._desc.anchorX = 0
--        barContainer._desc.anchorY = 0
        barContainer._desc.x = 20
        barContainer._desc.y = opt.descY
        barContainer._desc:setFillColor(unpack(opt.descColor))
        barContainer:insert(barContainer._desc)  
    end
